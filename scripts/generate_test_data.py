...
# --- Configuration ---
NUM_ROWS = 2000               # Testing file size (2,000 data points)
...
# Apply the timezone offset to the start_time (using a different start time for test file)
start_time = datetime(2023, 1, 2, 12, 0, 0, tzinfo=TZ_OFFSET)
...
# Define motion segments (scaled down 5x from the 10,000-row training set):
# Maintains the 60% active / 40% rest ratio.
SEGMENTS = {
    # Block 1: Initial Activity (0 - 600 rows)
    'walk_1': (0, 500),      # Initial Steady Walking (500 rows)
    'turn_1': (500, 600),    # Initial Sharp Turn (100 rows)

    # Block 2: Long Rest/Gap (600 - 1400 rows) - Creates the flat line in the plot (800 rows)
    'long_rest': (600, 1400),

    # Block 3: Final Activity (1400 - 2000 rows)
    'walk_2': (1400, 1900),  # Second Walking Phase (500 rows)
    'jump_2': (1900, NUM_ROWS),  # Quick Jump/Impact (100 rows)
}
...
# Gyro Z amplitude replicated from training script
...
# Phase C: Long Rest (600-1400) - Data is at the base signal (Accel_z=9.8, others=0.0)
...
segment_length = idx_end - idx_start # 100 rows
...
# Compute the median values (for verification only)
...
file_name = f'data/model_output/synthetic_testing_data_{timestamp_str}.csv'

# Save the full 2,000 rows
final_df.to_csv(file_name, index=False)

print(f"✅ Testing data generation complete. The full {len(final_df)} rows were saved to the file:\n   '{file_name}'")
print("\n--- Calculated Statistical Medians (for verification) ---")
